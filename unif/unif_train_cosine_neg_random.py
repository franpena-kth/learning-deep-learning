...
"""
A UNIF variant that just trains by maximizing the distance between a
code embedding an the embedding of a negative description.
It does *not* minimize the distance between the code embedding and its
corresponding description embedding
"""
...
def train_cycle(use_wandb=True):
    ...
    if use_wandb:
        wandb.init(project='code-search', name='unif-cosine-neg-random', reinit=True)
        config = wandb.config
